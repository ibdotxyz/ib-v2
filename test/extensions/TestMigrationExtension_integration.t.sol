...
contract MigrationExtensionIntegrationTest is Test, Common {
    ...
    function testMigrate3() public {
        ...
        additionalSupplies[0] =
            MigrationExtension.AdditionalSupply({market: 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE, amount: 10e18});

        extension.migrate{value: 15e18}(v1SupplyMarkets, v1BorrowMarkets, additionalSupplies); // will refund 5
        vm.stopPrank();

        assertEq(IERC20(ibUsdtV1).balanceOf(user1), 0);
        assertEq(IBTokenV1Interface(ibDaiV1).borrowBalanceStored(user1), 0);
        assertEq(ib.getSupplyBalance(user1, WETH), 10e18);
    }
