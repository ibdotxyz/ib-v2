...
contract MigrationExtension is Pausable, ReentrancyGuard, Ownable2Step, DeferLiquidityCheckInterface {
    ...
    function onDeferredLiquidityCheck(bytes memory encodedData) external override {
        ...
        for (uint256 i = 0; i < additionalSupply.length;) {
            uint256 supplyAmount = additionalSupply[i].amount;
            if (additionalSupply[i].market == ETH) {
                ...
                ironBank.supply(initiator, initiator, additionalSupply[i].market, supplyAmount);
            }
        ...
        // Refund unused native token back to user if any.
        if (unusedNativeToken > 0) {
            (bool sent,) = initiator.call{value: unusedNativeToken}("");
